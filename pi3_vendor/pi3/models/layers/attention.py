...
try:
    ...
    FLASH_ATTN_PACKAGE_AVAILABLE = True
    FLASH_ATTN_IMPORT_ERROR = None
except Exception as exc:  # pragma: no cover - depends on host native packages
    _flash_attn_func = None
    FLASH_ATTN_PACKAGE_AVAILABLE = False
    FLASH_ATTN_IMPORT_ERROR = f"{type(exc).__name__}: {exc}"
...
def _is_half_cuda_no_mask(q: Tensor, k: Tensor, v: Tensor, attn_mask=None) -> bool:
    return (
        attn_mask is None
        and q.is_cuda
        and k.is_cuda
...
def _flash_attn_package_attention(q: Tensor, k: Tensor, v: Tensor, *, dropout_p: float = 0.0) -> Tensor:
    """Run flash-attn package attention for tensors shaped [B, H, N, D]."""

    if _flash_attn_func is None:
        raise RuntimeError("flash_attn package is not available")
...
def _try_flash_attn_package(q: Tensor, k: Tensor, v: Tensor, *, dropout_p: float = 0.0) -> Tensor | None:
    if not _is_half_cuda_no_mask(q, k, v) or _flash_attn_func is None:
        return None
    try:
        return _flash_attn_package_attention(q, k, v, dropout_p=dropout_p)
    except Exception as exc:  # pragma: no cover - hardware/native-kernel dependent
        _LOGGER.warning("flash_attn package failed; falling back to PyTorch SDPA: %s", exc)
        return None


def _torch_flash_sdpa_is_safe(q: Tensor, k: Tensor, v: Tensor, attn_mask=None) -> bool:
    if not _is_half_cuda_no_mask(q, k, v, attn_mask):
        return False
    try:
...
def _pytorch_sdpa(
    ...
    *,
    attn_mask=None,
    dropout_p: float = 0.0,
    allow_flash_backend: bool = True,
) -> Tensor:
    if allow_flash_backend and _torch_flash_sdpa_is_safe(q, k, v, attn_mask):
        try:
            with nn.attention.sdpa_kernel(SDPBackend.FLASH_ATTENTION):
                return scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p)
        except Exception as exc:  # pragma: no cover - hardware/native-kernel dependent
            _LOGGER.warning("PyTorch Flash SDPA failed; falling back to math/efficient SDPA: %s", exc)

    with nn.attention.sdpa_kernel([SDPBackend.MATH, SDPBackend.EFFICIENT_ATTENTION]):
        return scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p)


def _no_mask_attention(q: Tensor, k: Tensor, v: Tensor, *, dropout_p: float = 0.0) -> Tensor:
    package_result = _try_flash_attn_package(q, k, v, dropout_p=dropout_p)
    if package_result is not None:
        return package_result
    return _pytorch_sdpa(q, k, v, dropout_p=dropout_p, allow_flash_backend=True)


def _masked_or_bias_attention(q: Tensor, k: Tensor, v: Tensor, *, attn_mask=None, dropout_p: float = 0.0) -> Tensor:
    return _pytorch_sdpa(q, k, v, attn_mask=attn_mask, dropout_p=dropout_p, allow_flash_backend=False)
...
class FlashAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        if attn_bias is None:
            x = _no_mask_attention(q, k, v)
        else:
            x = _masked_or_bias_attention(q, k, v, attn_mask=attn_bias)
...
class PRopeFlashAttention(AttentionRope):
    def forward(self, x: Tensor, extrinsics, H, W, patch_h, patch_w, K=None, attn_mask=None) -> Tensor:
        ...
        if attn_mask is None:
            ...
            x = _masked_or_bias_attention(q, k, v, attn_mask=attn_mask)
...
class FlashCrossAttentionRope(CrossAttentionRope):
    def forward(self, query: Tensor, key: Tensor, value: Tensor, attn_bias=None, qpos=None, kpos=None) -> Tensor:
        ...
        if attn_bias is None:
            x = _no_mask_attention(q, k, v, dropout_p=dropout_p)
        else:
            x = _masked_or_bias_attention(q, k, v, attn_mask=attn_bias, dropout_p=dropout_p)
